...
| ID | Item | Status | Notes |
|---|---|---|---|
| TD-1 | Monolith refactor | ✅ Done | All inline page functions extracted, including the renderPage orchestrator (extracted as StaffRouter in Phase 4 follow-up sprint). Monolith down from 8,219 → 5,332 lines (-35.1%). Four-layer Context architecture (DataContext, ActionsContext, UIContext, AuthContext) wired with useMemo. 16 pages + StaffRouter + DetailView extracted to src/features/. Bundle 433KB / 113KB gzipped. Component testing (31 tests) + Storybook-alternative catalogue + 100% context migration delivered in follow-up sprint. |
