  ...
  function Dashboard() {
    ...
    return (<div>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
        ...
        <KPI label="Total Disbursed" value={fmt.cur(totalDisbursed)} accent={C.green} />
        <KPI label="Pipeline" value={fmt.cur(pipeAmt)} sub={`${pipeline.length} pending`} accent={C.blue} />
        <KPI label="Arrears" value={fmt.cur(arrAmt)} sub={`${arrLoans.length} accounts in arrears`} accent={C.red} />
        <KPI label="ECL Provision" value={fmt.cur(ecl)} sub="IFRS 9 compliant" accent={C.purple} />
        <KPI label="Weighted Avg Rate" value={`${avgRate}%`} accent={C.amber} />
      </div>
      ...
      {(statutoryReports||[]).filter(r => r.status !== "Submitted").length > 0 && (
        <SectionCard title="NCR Statutory Reporting Deadlines" actions={<Btn size="sm" variant="ghost" onClick={() => setPage("statutory")}>View All {I.chev}</Btn>}>
          {(statutoryReports||[]).filter(r => r.status !== "Submitted").sort((a,b) => new Date(a.dueDate) - new Date(b.dueDate)).slice(0, 4).map(r => {
            ...
            const uc = days < 0 ? C.red : days <= 30 ? C.red : days <= 60 ? C.amber : C.green;
            return (
              <div key={r.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "10px 0", borderBottom: `1px solid ${C.border}` }}>
                <div style={{ width: 36, height: 36, border: `1px solid ${C.border}`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: urgencyColor(days), lineHeight: 1 }}>{days < 0 ? "!" : days}</div>
      ...
      {/* Alerts */}
      <SectionCard title="Recent Alerts & Early Warnings" actions={<Btn size="sm" variant="ghost" onClick={() => setPage("governance")}>View All</Btn>}>
        {alerts.slice(0, 6).map(a => (
          <div key={a.id} style={{ display:"flex", alignItems:"flex-start", gap:12, padding:"10px 0", borderBottom:`1px solid ${C.border}`, opacity:a.read?0.4:1 }}>
